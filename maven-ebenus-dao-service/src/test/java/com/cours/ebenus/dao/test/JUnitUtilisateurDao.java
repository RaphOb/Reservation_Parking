...
import com.cours.ebenus.dao.test.JUnitRoleDao;
...
public class JUnitUtilisateurDao {
    ...
    private static final int NB_UTILISATEURS_LIST = 15;
    ...
    private static final int NB_UTILISATEURS_FIND_BY_NOM = 5;
    ...
    private static final String UTILISATEUR_FIND_BY_IDENTIFIANT_ROLE_UNAUTHORIZED = "Unauthorized";
    private static final int NB_UTILISATEURS_FIND_BY_IDENTIFIANT_ROLE_UNAUTHORIZED = 5;
    ...
    public void testCreateUpdateDeleteUtilisateur() {
        ...
        userCRUD = new Utilisateur("Mr", "Admin", "Admin", "admin@gmail.com", "passw0rd", roleCRUD);
        try {
            userCRUD = serviceFacade.getUtilisateurDao().createUtilisateur(userCRUD);
            log.debug("Duplicate userCRUD : " + userCRUD.getEmail());
    ...
    public void testFindByCriteria() {
        ...
        List<Utilisateur> utilisateursByIdentifiantRoleUnauthorized = serviceFacade.getUtilisateurDao().findUtilisateursByIdentifiantRole(UTILISATEUR_FIND_BY_IDENTIFIANT_ROLE_UNAUTHORIZED);
        ...
        if (utilisateursByIdentifiantRoleUnauthorized != null && !utilisateursByIdentifiantRoleUnauthorized.isEmpty()) {
            log.debug("NB_UTILISATEURS_FIND_BY_IDENTIFIANT_ROLE_UNAUTHORIZED: " + NB_UTILISATEURS_FIND_BY_IDENTIFIANT_ROLE_UNAUTHORIZED + " , utilisateursByIdentifiantRoleUnauthorized.size(): " + utilisateursByIdentifiantRoleUnauthorized.size());
            Assert.assertEquals(NB_UTILISATEURS_FIND_BY_IDENTIFIANT_ROLE_UNAUTHORIZED, utilisateursByIdentifiantRoleUnauthorized.size());
            verifyUsersDatas(utilisateursByIdentifiantRoleUnauthorized);
        } else {
            Assert.fail("Aucun utilisateur avec le rôle '" + UTILISATEUR_FIND_BY_IDENTIFIANT_ROLE_UNAUTHORIZED + "' n'a ete trouve dans votre base de données");
        }
